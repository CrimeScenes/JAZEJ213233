...
local function GetPredictedPosition(player, time)
            ...
            if isMouseOnTarget(TargetPlayer) then
                ...
                for _, part in pairs(bodyParts) do
                    ...
                    if onScreen and isWithinBoxFOV(part.Position) then
                        ...
                        local predictedPos = GetPredictedPosition(TargetPlayer, 0.1)  -- 0.1 seconds prediction
                        local predictedScreenPos, onScreenPredicted = Camera:WorldToViewportPoint(predictedPos)
                        if onScreenPredicted and isWithinBoxFOV(predictedPos) then
                            syncBoxWithTarget(predictedScreenPos)
